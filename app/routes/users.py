...
def profile():
    ...
    total_items = Item.query.join(List).filter(
        List.user_id == current_user.id).count()
    total_likes = db.session.query(func.count()).filter(
        likes_table.c.list_id == List.id, likes_table.c.is_like == True,
        List.user_id == current_user.id).scalar() or 0
    total_dislikes = db.session.query(func.count()).filter(
        likes_table.c.list_id == List.id, likes_table.c.is_like == False,
        List.user_id == current_user.id).scalar() or 0
    total_comentarios = Comment.query.join(List).filter(
        List.user_id == current_user.id).count()

    listas_usuario = List.query.filter_by(user_id=current_user.id).order_by(
        List.timestamp.desc()).limit(5).all()
    items_usuario = Item.query.join(List).filter(
        List.user_id == current_user.id).order_by(
            Item.id.desc()).limit(5).all()
